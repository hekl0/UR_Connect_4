...
public class Connect_4_AB_Minimax extends Connect_4_Algorithm<State, Integer> implements AB_MinimaxAlgorithm<State, Integer> {
    Connect_4_Model connect_4_model;
    int stateVisited;

    public Connect_4_AB_Minimax(Connect_4_Model connect_4_model) {
        this.connect_4_model = connect_4_model;
    }

    @Override
    public Integer MiniMax(State state, int alpha, int beta, boolean isMax) {
        this.stateVisited++;
        ...
        for (Integer action: connect_4_model.AvailableAction(state))
            if (isMax) {
                score = Math.max(score, MiniMax(connect_4_model.ResultForPlayer(state, action, 1), alpha, beta, !isMax));
                alpha = Math.max(alpha, score);
                if (score >= beta)
                    return score;
            } else {
                score = Math.min(score, MiniMax(connect_4_model.ResultForPlayer(state, action, 2),  alpha, beta, !isMax));
    ...
    public Integer findBestMove(State state) {
        ...
        for (Integer action: connect_4_model.AvailableAction(state)) {
            int temp = MiniMax(connect_4_model.ResultForPlayer(state, action, 2), alpha, beta, true);
            if (temp < score) {
                score = temp;
                bestMove = action;
            }
